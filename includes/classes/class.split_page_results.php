...
// previous button - not displayed on first page
      if ($this->current_page_number > 1) $display_links_string .= '<a href="' . href_link(basename($PHP_SELF), $parameters . $this->page_name . '=' . ($this->current_page_number - 1), $request_type) .'&cat_id='.$_REQUEST['cat_id'].'&subcat_id='.$_REQUEST['subcat_id'].'&color='.$_REQUEST['color'].'&size='.$_REQUEST['size'].'&Ident='.$_REQUEST['Ident'].'&pLimit='.$_REQUEST['pLimit'].'&flag='.$_REQUEST['flag'].'&keyword='.$_REQUEST['keyword'].'' . '" class="title" title=" ' . PREVNEXT_TITLE_PREVIOUS_PAGE . ' " style= "color: #3C3C3C;"><u  class=blog_title >' . 'Prev' . '</u></a>&nbsp;&nbsp;';
...
// previous window of pages
      if ($cur_window_num > 1) $display_links_string .= '<a href="' . href_link(basename($PHP_SELF), $parameters . $this->page_name . '=' . (($cur_window_num - 1) * $max_page_links), $request_type) .'&cat_id='.$_REQUEST['cat_id'].'&subcat_id='.$_REQUEST['subcat_id'].'&color='.$_REQUEST['color'].'&size='.$_REQUEST['size'].'&Ident='.$_REQUEST['Ident'].'&pLimit='.$_REQUEST['pLimit'].'&flag='.$_REQUEST['flag'].'&keyword='.$_REQUEST['keyword'].'' . '" class="title" title=" ' . sprintf(PREVNEXT_TITLE_PREV_SET_OF_NO_PAGE, $max_page_links) . ' " style= "color: #3C3C3C;"></a>';
...
// page nn button
      for ($jump_to_page = 1 + (($cur_window_num - 1) * $max_page_links); ($jump_to_page <= ($cur_window_num * $max_page_links)) && ($jump_to_page <= $this->number_of_pages); $jump_to_page++) {
        if ($jump_to_page == $this->current_page_number) {
          ...
          $display_links_string .= '&nbsp;<a href="' . href_link(basename($PHP_SELF), $parameters . $this->page_name . '=' . $jump_to_page, $request_type) .'&cat_id='.$_REQUEST['cat_id'].'&subcat_id='.$_REQUEST['subcat_id'].'&color='.$_REQUEST['color'].'&size='.$_REQUEST['size'].'&Ident='.$_REQUEST['Ident'].'&pLimit='.$_REQUEST['pLimit'].'&flag='.$_REQUEST['flag'].'&keyword='.$_REQUEST['keyword'].'' . '" class="title" title=" ' . sprintf(PREVNEXT_TITLE_PAGE_NO, $jump_to_page) . ' " style= "color: #3C3C3C;"><u  class=blog_title>' . $jump_to_page . '</u></a>&nbsp;';
        }
      }

// next window of pages
      if ($cur_window_num < $max_window_num) $display_links_string .= '<a href="' . href_link(basename($PHP_SELF), $parameters . $this->page_name . '=' . (($cur_window_num) * $max_page_links + 1), $request_type) .'&cat_id='.$_REQUEST['cat_id'].'&subcat_id='.$_REQUEST['subcat_id'].'&size='.$_REQUEST['size'].'&color='.$_REQUEST['color'].'&Ident='.$_REQUEST['Ident'].'&pLimit='.$_REQUEST['pLimit'].'&flag='.$_REQUEST['flag'].'&keyword='.$_REQUEST['keyword'].'' . '" class="title" title=" ' . sprintf(PREVNEXT_TITLE_NEXT_SET_OF_NO_PAGE, $max_page_links) . ' " style= "color: #3C3C3C;"></a>&nbsp;';

// next button

      if (($this->current_page_number < $this->number_of_pages) && ($this->number_of_pages != 1))
	   $display_links_string .= '&nbsp;<a href="' . href_link(basename($PHP_SELF), $parameters . 'page=' . ($this->current_page_number + 1), $request_type) .'&cat_id='.$_REQUEST['cat_id'].'&subcat_id='.$_REQUEST['subcat_id'].'&color='.$_REQUEST['color'].'&size='.$_REQUEST['size'].'&Ident='.$_REQUEST['Ident'].'&pLimit='.$_REQUEST['pLimit'].'&flag='.$_REQUEST['flag'].'&keyword='.$_REQUEST['keyword'].'' . '" class="title" title=" ' . PREVNEXT_TITLE_NEXT_PAGE . ' " style= "color: #3C3C3C;"><u class=blog_title >' . 'Next'  . '</u></a>&nbsp;';
